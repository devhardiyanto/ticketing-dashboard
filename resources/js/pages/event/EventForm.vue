...
const form = useForm({
  ...
  organization_id: props.initialData?.organization_id || props.parentEvent?.organization_id || user?.organization_id || '',
...
onMounted(async () => {
  ...
  if (form.terms) {
    const hydrated = await hydrateContent(form.terms);
    if (hydrated !== form.terms) {
      form.terms = hydrated;
      form.defaults('terms', hydrated);
    }
  }
});
...
<template>
  <form @submit.prevent="submit" class="space-y-4">
    <div class="space-y-4">
      ...
      <div class="space-y-2">
        <Field name="terms" :invalid="!!form.errors.terms">
          <FieldLabel>Terms & Conditions</FieldLabel>
          <FieldContent>
            <QuillEditor v-model="form.terms" :height="200" />
          </FieldContent>
